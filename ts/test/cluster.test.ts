...
import { Cluster, type NodeSpec } from "../src/cluster.js";
import { orderParameter, criticalCoupling, KuramotoBank } from "../src/kuramoto.js";
import type { Target } from "../src/agent.js";

function threeNodes(): NodeSpec[] {
  return [
...
function srn(targetRegion = "*"): string {
...
test("submit rejects an expression with no `not` clause (SRN Cor 2.1)", () => {
  ...
  assert.equal((r as any).error.kind, "no-negation-boundary");
});
...
test("committed step advances (occupied, not waiting — Thm occupancy)", () => {
  ...
  const r = c.submit(srn());
  assert.equal(r.ok, true);
  assert.ok((r as any).committedStep >= 1, "agent has taken at least one committed step");
});

test("persistence: an agent with a succession map keeps a live goal (Thm persistence)", () => {
  const c = new Cluster({ nodes: threeNodes() });
  const ring: Target[] = [[0.02, 0.02], [0.03, 0.02], [0.02, 0.03]];
  let i = 0;
  const succession = () => {
    i = (i + 1) % ring.length;
    return ring[i]!;
  ...
  assert.equal(r.ok, true);
  const agent = c.agent((r as any).agent)!;
  // drive it well past first completion; it must not retire (goal succession)
  for (let k = 0; k < 400; k++) (agent as any).step?.();
