...
test("coords surface: make/validate/shell/tuple all reachable from index", () => {
  ...
  const t: Coord = coordTuple(c as PartitionCoords);
...
test("SRN surface: parse, eval, trajectory codec reachable from index", () => {
  ...
  const r = evalExpr(g as any, { coords: makeCoords(2, 1, 0, 1) as PartitionCoords, trajectoryCount: 0 }, new Map());
  assert.equal(r.kind, "value");
  const decoded = decodeTrajectory(encodeTrajectory(g as any));
  assert.ok(!("error" in decoded));
});

test("agent surface: ProcessAgent satisfies the Agent interface", () => {
  const a: Agent = new ProcessAgent({
    id: agentId("api"),
    frame: makeCoords(1, 0, 0, 1) as PartitionCoords,
    loc: [0, 0],
    goal: [0.05, 0] as Target,
  });
...
test("market surface: yieldOf / separationCost / clearMarket / forcedUtilisation reachable", () => {
  ...
  const ags: AgentId[] = [agentId("x0"), agentId("x1")];
  const payoff = (a: AgentId, s: string) => (a === "x0" ? (s === "e0" ? 5 : 1) : s === "e1" ? 5 : 1);
  const { assignment, prices } = clearMarket(ags, slots, payoff, TICK);
  assert.equal(assignment.get(agentId("x0")), "e0");
  const p: Price = prices.get("e0")!;
  assert.ok(p >= 0);
...
test("node surface: SrnNode eval reachable from index", () => {
  const node = SrnNode.reference(makeCoords(1, 0, 0, 1) as PartitionCoords);
  const g = glyph({ name: "n", target: makeCoords(1, 0, 0, 1) as PartitionCoords, notGuard: "x", body: "b", toTarget: "*" });
  node.evaluate(g);
...
test("cluster surface: Cluster.submit returns a Yield union", () => {
  ...
  const y: Yield = c.submit(`|t : (2,1,0,+)| not { z } do { emit self.n } to { * }`);
