...
import { makeCoords, isCoordError, coordsEqual, shell } from "../src/coords.js";
import { encodeCoords, decodeCoords, encodeTrajectory, decodeTrajectory, } from "../src/srn/trajectory.js";
...
test("coord round-trip: decode(encode(coords)) == coords for all shells n<=6", () => {
    for (let n = 1; n <= 6; n++) {
        for (const c of shell(n)) {
            ...
            assert.ok(coordsEqual(d.coords, c));
        }
    }
});
...
test("decodeTrajectory recovers the exact target address", () => {
    ...
    assert.ok(coordsEqual(decoded.target, target));
});
test("replay immunity: re-submitting a glyph gives a different frame (self.M advanced)", () => {
    ...
    assert.equal(r1.value.self_M !== r2.value.self_M, true);
});
...
//# sourceMappingURL=trajectory.test.js.map