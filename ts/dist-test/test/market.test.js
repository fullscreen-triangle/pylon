...
import { agentId } from "../src/agent.js";
import { yieldOf, separationCost, clearMarket, forcedUtilisation, } from "../src/market.js";
const TICK = 1e-3;
function unitSlots(n) {
...
function payoffMatrix(nA, nS, seed = 12345) {
    const M = [];
    let s = seed;
    const rnd = () => {
    ...
    for (let i = 0; i < nA; i++) {
        const row = [];
        for (let j = 0; j < nS; j++)
            row.push(Math.round(rnd() * 1000) / 1000);
        M.push(row);
    }
    const ags = agents(nA);
    const idx = new Map(ags.map((a, i) => [a, i]));
    const payoff = (a, slot) => M[idx.get(a)][Number(slot.slice(1))];
    return { payoff, ags, M };
}
...
test("Three-way Equivalence: clearMarket optimum is yield-optimal, closed, and clearing (Thm 7.1)", () => {
    ...
    const ty = (p) => {
        const a = new Map(ags.map((ag, i) => [ag, `e${p[i]}`]));
        return yieldOf(a, ags, slots, payoff, TICK);
    };
    let yMax = -Infinity;
    for (const p of perms)
        yMax = Math.max(yMax, ty(p));
    ...
    for (let i = 0; i < n; i++) {
        for (let j = 0; j < n; j++) {
            ...
            alt.set(ags[i], sj);
            alt.set(ags[j], si);
            assert.ok(yieldOf(alt, ags, slots, payoff, TICK) <= base + TICK, "closure violated: a swap improved yield by > tau_0");
        }
    }
    ...
    for (const s of slots) {
        assert.equal(prices.get(s.id), separationCost(s.id, assignment, ags, slots, payoff, TICK), "price must equal separation cost");
    ...
    for (const a of ags) {
        ...
        for (const s of slots) {
            ...
            if (occ !== undefined)
                alt.set(occ, cur);
            assert.ok(yieldOf(alt, ags, slots, payoff, TICK) <= base + TICK, "clearing: no move raises yield by > tau_0");
        }
    }
});
