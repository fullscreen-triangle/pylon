...
export const defaultUtilisationCost: UtilisationCost = (v) => v * v;
...
export function yieldOf(
  ...
  payoff: PayoffFn,
  tick: number,
  utilisationCost: UtilisationCost = defaultUtilisationCost,
): number {
  let numerator = 0;
  for (const a of agents) {
    const s = assignment.get(a);
    if (s !== undefined) numerator += payoff(a, s);
  }
  ...
  for (const slot of slots) {
    if (used.has(slot.id)) denom += tick * slot.capacity * utilisationCost(1);
  }
  if (denom <= 0) return 0;
...
function payoffSum(
  ...
  for (const a of agents) {
    ...
    if (slot !== undefined) s += payoff(a, slot);
  }
  return s;
}
...
export function separationCost(
  ...
  payoff: PayoffFn,
  _tick: number,
  _utilisationCost: UtilisationCost = defaultUtilisationCost,
): Price {
  const base = payoffSum(assignment, agents, payoff);
  const remaining = slots.filter((s) => s.id !== slotId);
  if (remaining.length === 0) return base; // removing the only slot costs all payoff

  // reassign every agent currently on e to its best remaining slot
  const realloc = new Map<AgentId, string>(assignment);
  for (const a of agents) {
...
export function clearMarket(
  ...
  const assign = new Map<AgentId, string>();
  ...
  const occupantOf = (slotId: string): AgentId | undefined => {
    for (const b of agents) if (assign.get(b) === slotId) return b;
    return undefined;
  };
  ...
  while (improved && guard++ < maxIters) {
    ...
    let bestMove: { agent: AgentId; slot: string; swapWith?: AgentId } | null = null;
    ...
    if (bestMove) {
      ...
      if (bestMove.swapWith !== undefined) assign.set(bestMove.swapWith, prevA);
      improved = true;
    }
  }
...
export function forcedUtilisation(args: {
  ...
  capacity: number; // c
  tick: number;
  maxRate: number; // v_bar
  benefit?: (v: number) => number; // concave b(v); default log(1+v)
  utilisationCost?: UtilisationCost; // convex g_u; default v^2/(1-v/vbar)
  steps?: number;
}): number {
  const P = args.pressure;
  const c = args.capacity;
  const vbar = args.maxRate;
  const b = args.benefit ?? ((v: number) => Math.log(1 + v));
  const gu = args.utilisationCost ?? ((v: number) => (v * v) / (1 - v / vbar));
