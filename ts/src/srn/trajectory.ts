...
import { coordsFromTuple, isCoordError, type Coord, type PartitionCoords } from "../coords.js";
import { glyph, type Glyph } from "./expr.js";
...
export function encodeCoords(coords: PartitionCoords, bodyDigestHex: string): Trajectory {
  const channels = 4;
...
export function decodeCoords(
  traj: Trajectory,
): { coords: PartitionCoords; digestLow: number } | { error: string } {
  const r = traj.deltas.map((d) => Math.round(d / Q));
  if (r.length < 6) return { error: "trajectory too short" };
  const [n, l, mBias, sBit, dHi, dLo] = r as [number, number, number, number, number, number];
  const tuple: Coord = [n, l, mBias - 1024, sBit === 1 ? 1 : -1];
  const coords = coordsFromTuple(tuple);
  if (isCoordError(coords)) return { error: coords.message };
 ...
 */
export function encodeTrajectory(g: Glyph): Trajectory {
 ...
 */
export function decodeTrajectory(traj: Trajectory): Glyph | { error: string } {
