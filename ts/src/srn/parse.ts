...
import { coordsFromTuple, isCoordError, type Coord } from "../coords.js";
import { glyph, type Glyph } from "./expr.js";

/** Why a source string failed to parse. */
export type ParseError =
  | { kind: "no-negation-boundary" }
  | { kind: "malformed-srn"; at: number; message: string };

/** True if the parse produced an error rather than a glyph. */
export function isParseError(x: Glyph | ParseError): x is ParseError {
  return (x as ParseError).kind === "no-negation-boundary" || (x as ParseError).kind === "malformed-srn";
}
 ...
 */
export function parseSrn(source: string): Glyph | ParseError {
  const src = source;
...
function braceClause(text: string, keyword: string): string | null {
...
function normSpin(tok: string): number {
  if (tok === "+" || tok === "+1" || tok === "1") return 1;
  if (tok === "-" || tok === "-1") return -1;
  const n = Number(tok);
  return n >= 0 ? 1 : -1;
}
